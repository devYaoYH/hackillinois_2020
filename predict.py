...
def run_test(target_channels=None,dat_file=None):
	avail_channels, channel_dat = extract_data(data_file="train\\COOLCAT_20100815_055813_69_20100815_055813_695.hdf" if dat_file is None else dat_file, max_len=1000)
	...
	for ch in avail_channels if target_channels is None else target_channels:
		...
		expt_corr.append(pearsonr(Y,predicted)[0])
		corr_exp = sensors[ch].get_exp_corr()
		print(f"{ch} expected corr: {corr_exp}")
	...
	if (target_channels is None):
		...
		ret_channels = [ch for ch in target_channels if ch in avail_channels]
